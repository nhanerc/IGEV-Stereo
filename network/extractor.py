...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        model = nn.Sequential(
            self.conv1, self.norm1, self.relu, self.conv2, self.norm2, self.relu
        )
        y = model(x)
...
class MultiBasicEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, num_layers: int = 3) -> List[List[torch.Tensor]]:
        model = nn.Sequential(
            self.conv1, self.norm1, self.relu1, self.layer1, self.layer2, self.layer3
        )
        x = model(x)
...
class Feature(nn.Module):
    ...
    def forward(self, x):
        x = self.block0(self.act1(self.bn1(self.conv_stem(x))))
        x4 = self.block1(x)
        x8 = self.block2(x4)
        x16 = self.block3(x8)
        x32 = self.block4(x16)
