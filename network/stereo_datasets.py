...
class KITTI(StereoDataset):
    def __init__(
        self, aug_params=None, root="/data/KITTI/KITTI_2015", image_set="training"
    ):
        ...
        image1_list = sorted(
            glob(os.path.join(root_12, image_set, "colored_0/*_10.png"))
        )
        image2_list = sorted(
            glob(os.path.join(root_12, image_set, "colored_1/*_10.png"))
        )
        disp_list = (
            sorted(glob(os.path.join(root_12, "training", "disp_occ/*_10.png")))
            if image_set == "training"
            else [osp.join(root, "training/disp_occ/000085_10.png")] * len(image1_list)
        )

        root_15 = "/data/KITTI/KITTI_2015"
        image1_list += sorted(
            glob(os.path.join(root_15, image_set, "image_2/*_10.png"))
        )
        image2_list += sorted(
            glob(os.path.join(root_15, image_set, "image_3/*_10.png"))
        )
        disp_list += (
            sorted(glob(os.path.join(root_15, "training", "disp_occ_0/*_10.png")))
            if image_set == "training"
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root="/data/Middlebury", split="F"):
        ...
        image1_list = sorted(
            [
                os.path.join(root, f"training{split}", f"{name}/im0.png")
                for name in lines
            ]
        )
        image2_list = sorted(
            [
                os.path.join(root, f"training{split}", f"{name}/im1.png")
                for name in lines
            ]
        )
        disp_list = sorted(
            [
                os.path.join(root, f"training{split}", f"{name}/disp0GT.pfm")
                for name in lines
            ]
        )
